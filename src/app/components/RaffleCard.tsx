  ...
  return (
    <Link key={raffle.id} href={`/raffle/${raffle.id}`}>
      ...
      >
        ...
        <h3 className={`absolute bottom-4 right-4  font-semibold text-sm text-white ${(raffle.available == false) ? 'bg-neutral-900' : 'bg-main'} px-2 py-1 rounded-full`}>{(raffle.available == false) ? 'Próximamente' : `€ ${parseFloat(raffle.price_ticket).toFixed(2)}`} </h3>
