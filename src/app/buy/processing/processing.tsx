...
export default function ProcessingModule() {
  ...
  useEffect(() => {
    ...
    async function fetchPayment() {
      try {
        const res = await fetch(`/api/monei/directPayment?id=${paymentId}&rid=${raffleId}&user=${user?.uid}&direct=true`)
            ...
            {loadingD ? (
              ...
              <>
                <h2 className="text-xl font-semibold text-green-600">
                  ✅ Boleto reservado correctamente
                </h2>
                <p className="text-gray-600 mb-8">Tu pago ha sido aprobado.</p>
                <Link href={raffleId ? `/tickets` : `/tickets`} className='mt-8 px-5 py-2 bg-black rounded-md text-white font-semibold text-sm cursor-pointer'>Continuar</Link>
              </>
